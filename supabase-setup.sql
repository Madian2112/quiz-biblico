...
CREATE TABLE IF NOT EXISTS temas (
  ...
  descripcion TEXT,
  icono_clase VARCHAR(100), -- Clase de icono de librería (ej: 'book-open')
...
CREATE INDEX IF NOT EXISTS idx_preguntas_libro ON preguntas(libro);
CREATE INDEX IF NOT EXISTS idx_preguntas_capitulo ON preguntas(capitulo);
CREATE INDEX IF NOT EXISTS idx_preguntas_libro_capitulo ON preguntas(libro, capitulo);
CREATE INDEX IF NOT EXISTS idx_preguntas_busqueda_completa ON preguntas(tema_id, libro, capitulo);
...
CREATE INDEX IF NOT EXISTS idx_sesiones_usuario ON sesiones_juego(user_id);
CREATE INDEX IF NOT EXISTS idx_sesiones_fecha ON sesiones_juego(fecha_inicio DESC);
CREATE INDEX IF NOT EXISTS idx_sesiones_temas ON sesiones_juego USING GIN(temas_ids);
...
CREATE INDEX IF NOT EXISTS idx_resultados_pregunta ON resultados(pregunta_id);

-- Tabla para tracking de preguntas usadas por usuario (CACHÉ LOCAL)
CREATE TABLE IF NOT EXISTS preguntas_usadas (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES usuarios(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_preguntas_usadas_usuario ON preguntas_usadas(user_id);
CREATE INDEX IF NOT EXISTS idx_preguntas_usadas_filtros ON preguntas_usadas(user_id, tema_id, libro, capitulo_inicio, capitulo_fin);
CREATE INDEX IF NOT EXISTS idx_preguntas_usadas_pregunta ON preguntas_usadas(pregunta_id);
...
DROP TRIGGER IF EXISTS update_preguntas_updated_at ON preguntas;
...
DROP POLICY IF EXISTS "Los usuarios pueden insertar su propia información" ON usuarios;
...
-- Insertar libros bíblicos (Génesis como ejemplo)
INSERT INTO libros_biblicos (nombre, abreviatura, testamento, total_capitulos, orden) VALUES
('Génesis', 'Gn', 'Antiguo', 50, 1),
('Éxodo', 'Ex', 'Antiguo', 40, 2),
('Mateo', 'Mt', 'Nuevo', 28, 40),
('Juan', 'Jn', 'Nuevo', 21, 43)
ON CONFLICT (nombre) DO NOTHING;

-- Insertar preguntas de ejemplo para Génesis 41-46
...
-- Capítulo 41
((SELECT id FROM temas WHERE nombre = 'Interpretación de Sueños'), 'Génesis', 41, 
...
((SELECT id FROM temas WHERE nombre = 'Personajes'), 'Génesis', 41,
'¿Quién interpretó los sueños de Faraón?',
'José',
'fácil',
'Génesis 41:15-16',
'{"categoria": "jose"}'::jsonb),

((SELECT id FROM temas WHERE nombre = 'Gobierno'), 'Génesis', 41,
'¿Qué cargo le dio Faraón a José después de interpretar sus sueños?',
'Gobernador de Egipto, segundo después de Faraón',
'medio',
'Génesis 41:40-41',
'{"categoria": "autoridad"}'::jsonb),

((SELECT id FROM temas WHERE nombre = 'Números'), 'Génesis', 41,
'¿Cuántos años tenía José cuando fue presentado ante Faraón?',
'Treinta años',
'medio',
'Génesis 41:46',
'{"categoria": "edades"}'::jsonb),

-- Capítulo 42
((SELECT id FROM temas WHERE nombre = 'Familia'), 'Génesis', 42,
'¿Cuántos hermanos de José fueron a Egipto a comprar grano?',
'Diez hermanos (Benjamín se quedó)',
'fácil',
'Génesis 42:3',
'{"categoria": "hermanos"}'::jsonb),

((SELECT id FROM temas WHERE nombre = 'Historia'), 'Génesis', 42,
'¿Qué acusación hizo José a sus hermanos cuando llegaron a Egipto?',
'Los acusó de ser espías',
'medio',
'Génesis 42:9',
'{"categoria": "encuentro"}'::jsonb),

-- Capítulo 43
((SELECT id FROM temas WHERE nombre = 'Familia'), 'Génesis', 43,
'¿Quién se ofreció como garantía para traer a Benjamín a Egipto?',
'Judá',
'medio',
'Génesis 43:8-9',
'{"categoria": "responsabilidad"}'::jsonb),

((SELECT id FROM temas WHERE nombre = 'Bendiciones'), 'Génesis', 43,
'¿Qué hizo José cuando vio a Benjamín?',
'Se conmovió y tuvo que retirarse para llorar',
'medio',
'Génesis 43:30',
'{"categoria": "emociones"}'::jsonb)
